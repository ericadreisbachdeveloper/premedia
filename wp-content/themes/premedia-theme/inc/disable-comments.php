...
add_action(
    ...
    function () {
            ...
            /*
             * Why esc_url_raw():
             * REQUEST_URI is a URL path
             * esc_url_raw() sanitizes URLs for database/redirects (keeps the raw URL structure)
             * sanitize_text_field() would break URL characters like /, ?, &
             */ 
        if ( isset( $_SERVER['REQUEST_URI'] )
            && ( strpos( esc_url_raw( wp_unslash( $_SERVER['REQUEST_URI'] ) ), '/wp-comments-post.php' ) !== false
            || strpos( esc_url_raw( wp_unslash( $_SERVER['REQUEST_URI'] ) ), '/xmlrpc.php' ) !== false )
        ) {
